...
(function ($, window, document, undefined) {
  ...
  Calendar.prototype = {
    showHoverInfo: function (obj) { // hover 时显示当天信息
      ...
      var offset_t = $(obj).offset().top + (this.$calendar_today.height() - $(obj).height()) / 2  - 50;
      ...
      this.$calendarToday_date.text("双击日期设置为开奖日，再次双击则取消" );
      this.$calendarToday_week.text(_weekStr +"周");
    },
    ...
    inital: function () { // 初始化
      ...
      this.$calendarDate_item.bind('click', function (obj) {

        var _dateStr = $(obj).attr('data');
        var changeStr = _dateStr.substr(0, 4) + '-' + _dateStr.substr(4, 2) + '-' + _dateStr.substring(6);
        console.log(changeStr)
        if(clicked_datestr == changeStr){
          // 切换当前日期状态
          $.ajax({
            // cache: true,
            type: "post",
            url: "/jieshaoadmin/setjieshao",
            data: {
              "info": data
            },
