...
$("#try").click(function(e) { //试试手气
    e.preventDefault();
    stopsound = false;
    if(orplay()||souqi) { // 如果 在搅珠则返回
        return;
    }
    setTimeout(function(){souqi=false;},35000)
    souqi = true;
    ...
    $("#die").addClass("play");
    $(".status").css("background-image", "url(img/status.gif)  !important")
...
//把数级遍历添加 dom，播放聲音
var addinter, oldI, breckClond,addinter;
...
function addDOM(Arr, kaiji) { //arr 传入的数组 ，kaiji是否在搅珠/ t rue  /  false
    ...
    if(kaiji == false) {
        ...
        addinter = setInterval(function() {
            ...
            console.log("oldI " + oldI, "   i " + i,"    arr="+Arr)
...
//请求后台开奖数据
//http://tempvsd.andsmind.com80/six-api/smallSix/testCurrentVideoInfo.do
//var httpurl="http://192.168.1.103:8080/six-api/smallSix/testCurrentVideoInfo.do?"   // 本地
...
function loadAjax() {
    ...
    $.ajax({
        type: "get",
        url:"Number/read3?rannum="+Math.ceil(Math.random()*1000000+1000),
        async: true,
        dataType: "json",
        success: function(data) {
            ...
            if(data.type == 4) {
                ...
                for(var i = 0; i < ma.length; i += 3) {
                    ...
                    temporarycol.push(ma[i+2])
                }
                console.log("breckClond ="+breckClond)
                console.log("kaijiArr ="+kaijiArr);
                console.log("临时color  "+temporarycol);
                var temporaryhtml=""
                for(var k =0;k<breckClond;k++){
                    temporaryhtml+= "<li class='"+temporarycol[k]+"'>" + kaijiArr[k] + "</li>"
                }
                $(".number>ul").html(temporaryhtml);
                if(breckClond != kaijiArr.length && kaijiArr != 0 ) {
                    kaijiIn(kaijiArr)
                } else {
...
function nextopenTime(nextTime) {
    ...
    nexttimeinter = setInterval(function() {
        ...
        if(day < 0 || hours < 0 || fen < 0 || miao < 0) {}
        else if(day == 0 && hours == 0 && fen == 0 && miao < 1) {
            var textime = '准备开奖:  . . .'
